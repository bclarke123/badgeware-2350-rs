...
use super::texture::{content_rect, TexMap, Textures, EDGE, TRANSPARENT};
...
pub fn draw_lists(
    ...
    while i < a.len && j < b.len {
        if a.depth_at(i) >= b.depth_at(j) {
            draw_one(a.nth(i), half, x0, x1, textures, &mut stats);
            i += 1;
        } else {
            draw_one(b.nth(j), half, x0, x1, textures, &mut stats);
            j += 1;
        }
    }
    while i < a.len {
...
#[inline(always)]
fn draw_one(t: &ScreenTri, half: &mut [u8], x0: i32, x1: i32, textures: &Textures, stats: &mut RasterStats) {
