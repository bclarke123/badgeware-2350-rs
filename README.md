...
| `totp` | Six-digit authenticator: battery-backed RTC, secret in flash, seven-segment digits with a draining 30 s bar, battery gauge. Evergreen: a lost clock (battery swap, drawer time) re-syncs itself over NTP with the stored WiFi credentials — provisioning over USB serial is first-boot only. On the 2040 W on battery it duty-cycles: every 5 s tick is a full boot — draw, refresh, arm the RTC alarm, power off. |
