...
| `totp` | Six-digit authenticator: battery-backed RTC, secret in flash, seven-segment digits with a draining 30 s bar, battery gauge. Evergreen: a lost clock (battery swap, drawer time) re-syncs itself over NTP with the stored WiFi credentials — provisioning over USB serial is first-boot only. |
